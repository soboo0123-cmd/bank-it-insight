...
def get_gemini_insight(news_list):
    ...
    prompt = f"""
        ...
        1. <종합 브리핑>:
           ...
           - 오늘 뉴스 중 가장 중요한 **경쟁사들의 실질적인 액션(신상품 출시, AI 활용 등)**을 중심으로 3~4문장의 전략적 통찰을 제공하세요.
           ...
           - 출력 결과물에 `🔗🔗`와 같이 기호가 연속해서 나타나지 않도록 최종 검토하여 작성하세요.
    
        2. <카테고리별 핵심 뉴스 요약>:
           - 아래 4가지 카테고리로 기사를 분류하되, **각 카테고리당 가장 중요한 기사를 3~4개씩만 엄선**하여 배치하세요.
             ① 경영전략 ② 신상품 ③ IT(기술/보안) ④ 기타
           
           - **[선별 기준]**: 
             1) <종합 브리핑>에서 언급된 핵심 사건과 연관된 기사를 최우선으로 배치하세요.
             2) 단순 홍보성 기사보다는 '신상품 출시', 'AI 활용', '기술 도입', '규제 대응' 등 전략적 가치가 높은 뉴스 위주로 선택하세요.
             
           - **[레이아웃]**:
             - 카테고리 제목(<h3>) 옆에 [핵심 트렌드]를 한 줄로 요약하세요. '<div style='background:#f8f9fa;'>'
             - 선택된 기사는 <ul>과 <li>를 사용하며, 제목에 원문 링크를 삽입하세요.
             - **[중요]** 각 기사의 내용(desc)은 **데이터로 제공된 원문을 임의로 요약하거나 수정하지 말고 텍스트 그대로(As-is) 전체를 표시**하세요. 네이버에서 전달받은 <br> 태그 등 HTML 서식도 그대로 유지해야 합니다.
